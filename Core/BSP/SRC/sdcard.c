...
void SDSPI_Init(void){  // SPI初始化
    spi_parameter_struct sdc_init;
    // 相关模块时钟设置
    rcu_periph_clock_enable(SDC_RCUP);
    rcu_periph_clock_enable(SDEN_RCUP);
    rcu_periph_clock_enable(SDC_RCUS);
    rcu_periph_clock_enable(RCU_AF);
    // GPIO设定
    // CLK 和 MOSI （mcu输出模式的引脚）设定为复用推挽模式
    gpio_init(SDC_PORT,GPIO_MODE_AF_PP,GPIO_OSPEED_50MHZ,SDC_CLK|SDC_MOSI);
    // MISO （mcu输入） 设定为浮动输入模式
    gpio_init(SDC_PORT,GPIO_MODE_IN_FLOATING,GPIO_OSPEED_50MHZ,SDC_MISO);
    // SD spi片选引脚设定推挽输出模式
    gpio_init(SDEN_PORT,GPIO_MODE_OUT_PP,GPIO_OSPEED_50MHZ,SDC_CS);
    // 失能sdcard片选
    SDC_DISABLE
    // 设定SPI
    ...
    spi_init(SDC_SPIN,&sdc_init);                       // 初始化spi1
    // 设定spi crc
    spi_crc_polynomial_set(SDC_SPIN,7);
    // 开启crc
    // spi_crc_on(SDC_SPIN);
    // 使能spi
    spi_enable(SDC_SPIN);
...
uint8_t SDSPI_RWbyte(uint8_t txData){
    while(RESET == spi_i2s_flag_get(SDC_SPIN,SPI_FLAG_TBE)){
    }
    spi_i2s_data_transmit(SDC_SPIN,txData);
    while(RESET == spi_i2s_flag_get(SDC_SPIN,SPI_FLAG_RBNE)){
    }
    return spi_i2s_data_receive(SDC_SPIN);
}
...
void SDSPI_SetSpeed(uint8_t SDSPI_BAUD){
    ...
    SDC_DISABLE
    spi_parameter_struct sdc_init;
    spi_disable(SDC_SPIN);
    spi_i2s_deinit(SDC_SPIN);   // 复位SPI
    // 设定SPI
    sdc_init.trans_mode     = SPI_TRANSMODE_FULLDUPLEX; // 设置为全双工模式
    sdc_init.device_mode    = SPI_MASTER;               // 设置为主机模式
    sdc_init.frame_size     = SPI_FRAMESIZE_8BIT;       // 设置为8bit通信模式
    ...
    spi_init(SDC_SPIN,&sdc_init);   // 重新初始化
    // 使能片选
    SDC_ENABLE
}
...
uint8_t SD_GetResponse(uint8_t Response){
    ...
    // 等待得到回应
    while((SDSPI_RWbyte(0xFF) != Response)&&count){
        count--;
    }
    if(count == 0){
